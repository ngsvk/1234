...
export default function PortalStaffRatings() {
  const { user, setSidebarOpen } = useOutletContext<PortalStaffOutletContext>();

  useEffect(() => {
    document.title = 'Staff Portal · Students Rating';
  }, []);
  ...
  return (
    <>
      ...
      <Card className="shadow-sm border bg-card/80 mt-4">
        <CardHeader>
          <CardTitle className="text-base">Students Rating</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4 text-sm">
          <div className="flex flex-wrap gap-4 text-xs">
            <div>
              <p className="text-muted-foreground">Average Class Rating</p>
              <p className="text-foreground font-semibold">4.6 / 5</p>
            </div>
            <div>
              <p className="text-muted-foreground">Feedback Forms Collected</p>
              <p className="text-foreground font-semibold">120</p>
            </div>
          </div>

          <div className="mt-4 border-t pt-4 space-y-2 text-xs">
            <p className="text-muted-foreground">Recent feedback</p>
            <div className="space-y-1">
              <p>• "Explains concepts very clearly and gives practical examples."</p>
              <p>• "Class pace is comfortable and doubts are resolved in time."</p>
            </div>
          </div>
        </CardContent>
      </Card>
