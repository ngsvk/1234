...
export default function PortalStaffExam() {
  const { user, setSidebarOpen } = useOutletContext<PortalStaffOutletContext>();

  useEffect(() => {
    document.title = 'Staff Portal · Exam';
  }, []);
  ...
  return (
    <>
      ...
      <Card className="shadow-sm border bg-card/80 mt-4">
        <CardHeader>
          <CardTitle className="text-base">Exam Overview</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4 text-sm">
          <div className="flex flex-wrap gap-4 text-xs">
            <div>
              <p className="text-muted-foreground">Next Exam</p>
              <p className="text-foreground font-semibold">Midterm • Jan 10</p>
            </div>
            <div>
              <p className="text-muted-foreground">Papers to Evaluate</p>
              <p className="text-foreground font-semibold">45 Pending</p>
            </div>
          </div>

          <div className="mt-4 border-t pt-4">
            <p className="text-xs text-muted-foreground mb-2">Upcoming exams</p>
            <div className="space-y-2 text-xs">
              <div className="flex items-center justify-between">
                <span>XI - Computer Science Unit Test</span>
                <span className="text-muted-foreground">Jan 10 • 9:00 AM</span>
              </div>
              <div className="flex items-center justify-between">
                <span>XI - Computer Science Lab Practical</span>
                <span className="text-muted-foreground">Jan 15 • 11:00 AM</span>
              </div>
            </div>
          </div>
        </CardContent>
